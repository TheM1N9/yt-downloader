...
import type { TiktokVideoInfo } from "./tiktok-url"
...
export async function getTiktokVideoInfo(normalizedUrl: string): Promise<TiktokVideoInfo> {
  const info = execYtDlpJson(normalizedUrl)
  const id = (info.id as string) || "unknown"
  const title = (info.title as string) || (info.description as string) || "TikTok video"
...
export function createTiktokDownloadStream(normalizedUrl: string): Readable {
  const args = [...baseArgs(), "-f", "best", "-o", "-", normalizedUrl]
  const proc = spawn(YT_DLP_PATH, args, spawnOpts())
  proc.stderr?.on("data", (data: Buffer) => {
    console.error("yt-dlp [tiktok]:", data.toString().trim())
  })
  proc.on("error", (err) => {
