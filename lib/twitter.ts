...
import type { TwitterVideoInfo } from "./twitter-url"
...
export async function getTwitterVideoInfo(normalizedUrl: string): Promise<TwitterVideoInfo> {
  const info = execYtDlpJson(normalizedUrl)
  const id = (info.id as string) || "unknown"
  const title = (info.title as string) || (info.description as string) || "Twitter video"
...
export function createTwitterDownloadStream(normalizedUrl: string): Readable {
  const args = [...baseArgs(), "-f", "best", "-o", "-", normalizedUrl]
  const proc = spawn(YT_DLP_PATH, args, spawnOpts())
  proc.stderr?.on("data", (data: Buffer) => {
    console.error("yt-dlp [twitter]:", data.toString().trim())
  })
  proc.on("error", (err) => {
